"""Offline Code-39 barcode + synthetic package-label rendering (shared util).

Used by the demo mobile-intake asset generator and by tests (real barcode +
real OCR E2E). Deterministic: the same input always yields the same bytes.
No network access is ever required. The line-growing Code-39 renderer was
validated against pyzbar/zbar.
"""
...
def code39_image(value: str, scale: int = 6, height: int = 240, margin: int = 90) -> np.ndarray:
    """Render a Code-39 barcode as a BGR image (offline)."""
    import cv2
...
def package_label_bytes(
    *,
    ...
    demo_watermark: str | None = None,
    format_: str = ".jpg",
) -> bytes:
    """Render a close-up package label + barcode and return encoded bytes."""
    import cv2
    ...
    if demo_watermark:
        cv2.putText(
            label, demo_watermark, (140, 150), cv2.FONT_HERSHEY_SIMPLEX, 3.4,
            (120, 120, 140), 7, cv2.LINE_AA,
        )
    y = 340
    ...
    ok, buf = cv2.imencode(format_, label)
    if not ok:
